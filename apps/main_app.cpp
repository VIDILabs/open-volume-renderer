...
main(int ac, const char** av)
{
  ...
  if (ac < 2) {
    ...
    ovr::scene::Texture volume_raw;
    volume_raw.type = ovr::scene::Texture::VOLUME_TEXTURE;
    volume_raw.volume.volume = scene.instances[0].models[0].volume_model.volume;
    scene.textures.push_back(volume_raw);
    const int32_t volume_raw_id = scene.textures.size() - 1;
