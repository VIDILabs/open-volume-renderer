...
bool colormap::has(const char* name) {
	return data.find(std::string(name)) != data.end();
}
const std::vector<colormap::color_t>& colormap::get(const char* name) {
  return *data.at(std::string(name));
}
