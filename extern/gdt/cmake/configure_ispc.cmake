...
if (NOT ISPC_EXECUTABLE)
  ...
  if (NOT ISPC_EXECUTABLE)
    ...
    message("")
    message("This version of VNR expects you to have a binary install of ISPC minimum version ${ISPC_VERSION_REQUIRED}, and expects it to be found in 'PATH' or in the sibling directory to where the OSPRay source are located. Please go to https://ispc.github.io/downloads.html, select the binary release for your particular platform, and unpack it to ${PROJECT_SOURCE_DIR}/../")
    message("")
...
if (NOT (ISPC_ADDRESSING STREQUAL "32" OR
         ISPC_ADDRESSING STREQUAL "64"))
  message(FATAL_ERROR "ISPC_ADDRESSING must be set to either '32' or '64'!")
endif()
...
macro(configure_ispc_isa)
  ...
  option(VNR_ISA_SSE4 "Enables SSE4 ISA." ON)
  option(VNR_ISA_AVX "Enables AVX ISA." ON)
  option(VNR_ISA_AVX2 "Enables AVX2 ISA." ON)
  option(VNR_ISA_AVX512KNL "Enables AVX512 ISA for Knights Landing." OFF)
  option(VNR_ISA_AVX512SKX "Enables AVX512 ISA for Skylake." ON)

  if (VNR_ISA_AVX512KNL AND VNR_ISA_AVX512SKX)
  ...
  unset(ISPC_TARGET_LIST_4)
  unset(ISPC_TARGET_LIST_8)
  unset(ISPC_TARGET_LIST_16)

  if (VNR_ISA_SSE4)
    set(ISPC_TARGET_LIST ${ISPC_TARGET_LIST} sse4)
    set(ISPC_TARGET_LIST_4 ${ISPC_TARGET_LIST_4} sse4)
    message(STATUS "VNR SSE4 ISA target enabled.")
  endif()

  if (VNR_ISA_AVX)
    set(ISPC_TARGET_LIST ${ISPC_TARGET_LIST} avx)
    set(ISPC_TARGET_LIST_8 ${ISPC_TARGET_LIST_8} avx)
    message(STATUS "VNR AVX ISA target enabled.")
  endif()

  if (VNR_ISA_AVX2)
    set(ISPC_TARGET_LIST ${ISPC_TARGET_LIST} avx2)
    set(ISPC_TARGET_LIST_8 ${ISPC_TARGET_LIST_8} avx2)
    message(STATUS "VNR AVX2 ISA target enabled.")
  endif()

  if (VNR_ISA_AVX512KNL)
    set(ISPC_TARGET_LIST ${ISPC_TARGET_LIST} avx512knl-i32x16)
    set(ISPC_TARGET_LIST_16 ${ISPC_TARGET_LIST_16} avx512knl-i32x16)
    message(STATUS "VNR AVX512KNL ISA target enabled.")
  endif()

  if (VNR_ISA_AVX512SKX)
    set(ISPC_TARGET_LIST ${ISPC_TARGET_LIST} avx512skx-i32x16)
    set(ISPC_TARGET_LIST_16 ${ISPC_TARGET_LIST_16} avx512skx-i32x16)
    message(STATUS " AVX512SKX ISA target enabled.")
  endif()
  ...
  # of different widths.
  foreach (TARGET_LIST ISPC_TARGET_LIST_4
                       ISPC_TARGET_LIST_8
                       ISPC_TARGET_LIST_16)
