...
struct GDT_INTERFACE vec_t<T, 1> {
  ...
  inline __both__ size_t long_product() const { return v; }
...
struct GDT_INTERFACE vec_t<T, 2> {
  ...
  inline __both__ size_t long_product() const { return (size_t)x * y; }
...
struct GDT_INTERFACE vec_t<T, 3> {
  ...
  /*! swizzle ... */
  inline __both__ vec_t<T, 3> yzx() const
  {
    return vec_t<T, 3>(y, z, x);
  }
  ...
  inline __both__ size_t long_product() const { return (size_t)x * y * z; }
...
struct GDT_INTERFACE vec_t<T, 4> {
  ...
  inline __both__ vec_t(const T& t) : x(t), y(t), z(t), w(t) {}
  inline __both__ vec_t(const vec_t<T, 3>& xyz, const T& _w) : x(xyz.x), y(xyz.y), z(xyz.z), w(_w) {}
  inline __both__ vec_t(const T& _x, const T& _y, const T& _z, const T& _w) : x(_x), y(_y), z(_z), w(_w) {}

#ifdef __CUDACC__
  inline __both__ vec_t(const float4& v) : x(v.x), y(v.y), z(v.z), w(v.w) {}
  inline __both__ vec_t(const int4& v) : x(v.x), y(v.y), z(v.z), w(v.w) {}
  ...
  /*! construct 3-vector from 3-vector of another type */
  template<typename OT>
  inline __both__ explicit vec_t(const vec_t<OT, 4>& o) : x(o.x), y(o.y), z(o.z), w(o.w)
  {
  }
  inline __both__ vec_t(const vec_t<T, 4>& o) : x(o.x), y(o.y), z(o.z), w(o.w) {}

  /*! swizzle ... */
  inline __both__ const vec_t<T, 3>& xyz() const
  {
    return (const vec_t<T, 3>&)(*this);
  }
  inline __both__ vec_t<T, 3>& xyz()
  {
    return (vec_t<T, 3>&)(*this);
  }
  ...
  template<typename OT, typename Lambda>
  static inline __both__ vec_t<T, 4> make_from(const vec_t<OT, 4>& v, const Lambda& lambda)
  {
    return vec_t<T, 4>(lambda(v.x), lambda(v.y), lambda(v.z), lambda(v.w));
  }

  inline __both__ size_t long_product() const { return (size_t)x * y * z * w; }
...
template<typename T>
inline __both__
vec_t<T, 3>::vec_t(const vec_t<T, 4>& v)
  : x(v.x), y(v.y), z(v.z)
{
}
...
template<typename T>
inline __both__ typename long_type_of<T>::type
area(const vec_t<T, 2>& v)
{
...
template<typename T>
inline __both__ typename long_type_of<T>::type
volume(const vec_t<T, 4>& v)
{
  return (typename long_type_of<T>::type)(v.x) * (typename long_type_of<T>::type)(v.y) *
         (typename long_type_of<T>::type)(v.z) * (typename long_type_of<T>::type)(v.w);
}

template<typename T>
inline __both__ typename long_type_of<T>::type
area(const vec_t<T, 3>& v)
{
  return T(2) * ((typename long_type_of<T>::type)(v.x) * v.y + (typename long_type_of<T>::type)(v.y) * v.z +
                 (typename long_type_of<T>::type)(v.z) * v.x);
}

/*! vector cross product */
template<typename T>
inline __both__ vec_t<T, 3>
cross(const vec_t<T, 3>& a, const vec_t<T, 3>& b)
{
...
template<typename T>
inline __both__ T
dot(const vec_t<T, 2>& a, const vec_t<T, 2>& b)
{
...
template<typename T>
inline __both__ T
dot(const vec_t<T, 3>& a, const vec_t<T, 3>& b)
{
...
/*! vector cross product */
template<typename T>
inline __both__ vec_t<T, 2>
normalize(const vec_t<T, 2>& v)
{
...
template<typename T>
inline __both__ T
length(const vec_t<T, 3>& v)
{
  return gdt::overloaded::sqrt(dot(v, v));
}

template<typename T>
inline __gdt_host std::ostream&
operator<<(std::ostream& o, const vec_t<T, 1>& v)
{
  o << "(" << v.x << ")";
  return o;
}
...
template<typename T>
inline __gdt_host std::ostream&
operator<<(std::ostream& o, const vec_t<T, 2>& v)
{
  o << "(" << v.x << "," << v.y << ")";
  return o;
}
...
template<typename T>
inline __gdt_host std::ostream&
operator<<(std::ostream& o, const vec_t<T, 3>& v)
{
  o << "(" << v.x << "," << v.y << "," << v.z << ")";
  return o;
}
...
template<typename T>
inline __gdt_host std::ostream&
operator<<(std::ostream& o, const vec_t<T, 4>& v)
{
  o << "(" << v.x << "," << v.y << "," << v.z << "," << v.w << ")";
  return o;
}
