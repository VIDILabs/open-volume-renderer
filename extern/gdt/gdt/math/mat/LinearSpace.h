...
namespace gdt {
  ...
  template<typename T> struct GDT_INTERFACE LinearSpace2
  {
    ...
    inline __both__ const vector_t row0() const { return vector_t(vx.x,vy.x); }

    /*! returns second row of matrix */
    inline __both__ const vector_t row1() const { return vector_t(vx.y,vy.y); }

    /*! returns a particular column */
    inline __both__ const vector_t operator[](size_t col) const { return (&vx)[col]; }

    ////////////////////////////////////////////////////////////////////////////////
    /// Constants
    ////////////////////////////////////////////////////////////////////////////////

    inline __both__ LinearSpace2( ZeroTy ) : vx(zero), vy(zero) {}
    inline __both__ LinearSpace2( OneTy ) : vx(one, zero), vy(zero, one) {}

    /*! return matrix for scaling */
    static inline __both__ LinearSpace2 scale(const vector_t& s) {
      return LinearSpace2(s.x,   0,
                          0  , s.y);
    }

    /*! return matrix for rotation */
    static inline __both__ LinearSpace2 rotate(const scalar_t& r) {
  ...
  template<typename T> inline __both__ LinearSpace3<T>& operator *=( LinearSpace3<T>& a, const LinearSpace3<T>& b ) { return a = a * b; }
  template<typename T> inline __both__ LinearSpace3<T>& operator /=( LinearSpace3<T>& a, const LinearSpace3<T>& b ) { return a = a / b; }
  ...
  template<typename T> inline __both__ bool operator ==( const LinearSpace3<T>& a, const LinearSpace3<T>& b ) { return a.vx == b.vx && a.vy == b.vy && a.vz == b.vz; }
  template<typename T> inline __both__ bool operator !=( const LinearSpace3<T>& a, const LinearSpace3<T>& b ) { return a.vx != b.vx || a.vy != b.vy || a.vz != b.vz; }
