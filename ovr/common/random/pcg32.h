...
struct pcg32 {
	...
	__both__ pcg32() : state(PCG32_DEFAULT_STATE), inc(PCG32_DEFAULT_STREAM) {}

	/// Initialize the pseudorandom number generator with the \ref seed() function
	__both__ pcg32(uint64_t initstate, uint64_t initseq = 1u) { seed(initstate, initseq); }
	 ...
	 */
	__both__ void seed(uint64_t initstate, uint64_t initseq = 1) {
		state = 0U;
		inc = (initseq << 1u) | 1u;
		next_uint();
		state += initstate;
	...
	__both__ uint32_t next_uint(uint32_t bound) {
	 ...
	 * are made inbetween.
	 */
	__both__ void advance(int64_t delta_ = (1ll<<32)) {
		uint64_t
			cur_mult = PCG32_MULT,
			cur_plus = inc,
			acc_mult = 1u,
	...
	__both__ int64_t operator-(const pcg32 &other) const {
	...
	/// Equality operator
	__both__ bool operator==(const pcg32 &other) const { return state == other.state && inc == other.inc; }
...
#endif // OVR_OPTIX7_RANDOM_PCG32_H
