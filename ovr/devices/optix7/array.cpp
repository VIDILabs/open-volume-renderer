...
template<typename T>
Array1DScalarOptix7
CreateArray1DScalarOptix7(const std::vector<T>& input, cudaStream_t stream)
{
  static_assert(std::is_scalar<T>::value, "expecting a scalar type");
...
template Array1DScalarOptix7
CreateArray1DScalarOptix7<float>(const std::vector<float>& input, cudaStream_t stream);

// This is simply a local helper function
template<typename T>
Array1DScalarOptix7
CreateArray1DScalarOptix7(array_1d_scalar_t input, const char* data)
{
  static_assert(std::is_scalar<T>::value, "expecting a scalar type");
...
Array1DFloat4Optix7
CreateArray1DFloat4Optix7(const std::vector<vec4f>& input, cudaStream_t stream)
{
...
#define instantiate_create_array_3d_scalar(T) \
  template Array3DScalarOptix7 CreateArray3DScalarOptix7<T>(void* input, vec3i dims);
...
Array3DScalarOptix7
CreateArray3DScalarOptix7(array_3d_scalar_t array)
{
  Array3DScalarOptix7 output;
  std::shared_ptr<char[]> buffer;
  ...
  case VALUE_TYPE_DOUBLE:
    ...
    output = CreateArray3DScalarOptix7<float>(buffer.get(), array->dims);
    break;
  default: throw std::runtime_error("[Optix7] unexpected volume type ...");
  }
